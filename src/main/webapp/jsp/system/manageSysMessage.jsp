...
$(function() {
	...
	dataGrid = $('#dataGrid').datagrid({
		url : '${pageContext.request.contextPath}/sysMessageController/dataGrid.html',
		fitColumns : true,
		border : false,
		pagination : true,
		idField : 'id',
...
function addFun() {
	parent.$.modalDialog({
		...
		width : 400,
		height : 500,
		href : '${pageContext.request.contextPath}/sysMessageController/addPage.html',
		buttons : [ {
			text : '添加',
			handler : function() {
...
function editFun(id) {
	...
	parent.$.modalDialog({
		...
		width : 400,
		height : 500,
		href : '${pageContext.request.contextPath}/sysMessageController/editPage.html?id=' + id,
		buttons : [ {
			text : '编辑',
			handler : function() {
				parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
...
function deleteFun(id) {
	...
	parent.$.messager.confirm('询问', '您是否要删除当前信息？', function(b) {
		if (b) {
				...
				$.post('${pageContext.request.contextPath}/sysMessageController/removesysmessage.html', {
					id : id
				}, function(result) {
					if (result.success) {
...
function openFun(id) {
	...
	parent.$.messager.confirm('询问', '您是否要启用当前信息？', function(b) {
		if (b) {
				...
				$.post('${pageContext.request.contextPath}/sysMessageController/opensysmessage.html', {
					id : id
				}, function(result) {
					if (result.success) {
						parent.$.messager.alert('提示', result.msg, 'info');
...
function closeFun(id) {
	...
	parent.$.messager.confirm('询问', '您是否要停用当前信息？', function(b) {
		if (b) {
				...
				$.post('${pageContext.request.contextPath}/sysMessageController/closesysmessage.html', {
					id : id
				}, function(result) {
					if (result.success) {
						parent.$.messager.alert('提示', result.msg, 'info');
	...
	<div data-options="region:'north',title:'查询条件',border:false" style="height: 60px; overflow: hidden;">
		<form id="searchForm">
			<table class="table table-hover table-condensed" style="">
				<tr>
					<th>标题</th>
					<td><input name="name" id="name" placeholder="可以模糊查询登录名" class="span2" /></td>
				</tr>
			</table>
		</form>
	</div>
