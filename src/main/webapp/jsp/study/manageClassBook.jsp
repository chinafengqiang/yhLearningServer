...
$(function() {
	...
	dataGrid = $('#dataGrid').datagrid({
		url : '${pageContext.request.contextPath}/courseController/dataGridClassBook.html',
		fitColumns : true,
		border : false,
		pagination : true,
		idField : 'id',
...
function addFun() {
	parent.$.modalDialog({
		...
		width : 600,
		height : 500,
		href : '${pageContext.request.contextPath}/courseController/addClassBook.html',
		buttons : [ {
			text : '添加',
			handler : function() {
...
function editFun(id) {
	...
	parent.$.modalDialog({
		...
		width : 600,
		height : 500,
		href : '${pageContext.request.contextPath}/courseController/editClassBook.html?id=' + id,
		buttons : [ {
			text : '编辑',
			handler : function() {
				parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
...
function deleteFun(id) {
	...
	parent.$.messager.confirm('询问', '您是否要删除当前信息？', function(b) {
		if (b) {
				...
				$.post('${pageContext.request.contextPath}/courseController/removeClassBook.html', {
					id : id
				}, function(result) {
					if (result.success) {
