...
$(function() {
	...
	dataGrid = $('#dataGrid').datagrid({
		url : '${pageContext.request.contextPath}/coursewareController/dataGridCourseware.html',
		fitColumns : true,
		border : false,
		pagination : true,
		idField : 'id',
...
function addFun() {
	parent.$.modalDialog({
		...
		width : 600,
		height : 500,
		href : '${pageContext.request.contextPath}/coursewareController/addCourseware.html',
		buttons : [ {
			text : '添加',
			handler : function() {
...
function editFun(id) {
	...
	parent.$.modalDialog({
		...
		width : 600,
		height : 500,
		href : '${pageContext.request.contextPath}/coursewareController/editCourseware.html?id=' + id,
		buttons : [ {
			text : '编辑',
			handler : function() {
				parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
	...
	<div data-options="region:'north',title:'查询条件',border:false" style="height: 100px; overflow: hidden;">
		<form id="searchForm">
			<table class="table table-hover table-condensed" style="display: none;">
				<tr>
					<th>课件名称</th>
					<td><input name="name" id="name" placeholder="可以模糊查询" class="span2" /></td>
				</tr>
				<tr>
				<th>分类名称</th>
					<td><input name="categoryName" id="categoryName" placeholder="可以模糊查询" class="span2" /></td>
				</tr>
			</table>
		</form>
	</div>
